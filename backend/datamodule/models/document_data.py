...
class DocumentData(Model):
    def __init__(self, id: str = None, ocr_full_text: str = None, ocr_extracted_data: dict = None, layoutlm_full_text: str = None, layout_lm_data: dict = None):
        if id:
            self.id = id
        else:
            self.id = str(uuid4())
    ...
    def insert(self) -> tuple:
        values = (self.id, self.id, self.ocr_full_text, self.ocr_extracted_data, self.layoutlm_full_text, self.layout_lm_data)
        try:
            db.connect()
            # Execute insert
            ...
            if not tuple_document_data:
                raise InsertError("Failed to insert document data into database.")
    ...
    def from_tuple(tuple_data: tuple):
        ...
        if tuple_data:
            return DocumentData(
                id=tuple_data[0],
                ocr_full_text=tuple_data[1],
                ocr_extracted_data=tuple_data[2],
                layoutlm_full_text=tuple_data[3],
                layout_lm_data=tuple_data[4]
            )
        else:
            return None
